...
const UsersList = () => {
  ...
  return (
    ...
    {user && ( <div>
      ...
      <div>
        <button className="btn btn-submit" onClick={() => newUser() }  style={{padding:'20px',fontSize:'30px',marginTop:'10px',marginBottom:'10px'}}>Creat Users</button>
      </div>
          ...
          return (
            <>
              <tr key={index} className="table-secondary">
                ...
                <td>{value.phone}</td>
                <td>{value.date}</td>
                <td><button className="btn btn-info"  onClick={() => allocateLeads(value._id)} >Allocate</button></td>
              </tr>
            </>
          );
        })}
